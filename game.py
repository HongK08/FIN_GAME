...
def get_resource_path(relative_path):
    ...
    try:
        # PyInstaller에서 실행되는 경우, 리소스 경로는 _MEIPASS로 지정됨
        base_path = sys._MEIPASS
    except Exception:
        # 개발 환경에서는 현재 파일 경로를 기준으로 처리
        base_path = os.path.abspath(os.path.dirname(__file__))
    
    return os.path.join(base_path, relative_path)

# 보스 이미지와 미사일
boss_image = pygame.image.load(get_resource_path('image/BOSS.png'))
boss_missile_image = pygame.image.load(get_resource_path('image/BOSS_WEP.png'))

# 배경음악 및 효과음 파일 경로
background_music = get_resource_path('sounds/back_MS.mp3')
missile_sound = get_resource_path('sounds/MIS_LC.mp3')
boss_missile_sound = get_resource_path('sounds/spin.mp3')
explosion_sound = get_resource_path('sounds/BOOM.mp3')
boss_missile_explosion_sound = get_resource_path('sounds/MOI.mp3')
...
def initGame():
    ...
    # 그린 이미지 값 넣기
    background = pygame.image.load(get_resource_path('image/ST_BACK.jpg'))
    fighter = pygame.image.load(get_resource_path('image/PLA.png'))
    missile = pygame.image.load(get_resource_path('image/MISSL.png'))
    explosion = pygame.image.load(get_resource_path('image/EXP.png'))
...
def runGame():
    ...
    # 보스 이미지 크기 변환
    scaled_boss_image = pygame.transform.scale(boss_image, (200, 150))  # 보스 크기 조정 (200x150)
    scaled_boss_missile = pygame.transform.scale(boss_missile_image, (50, 50))  # 보스 미사일 크기 조정
    ...
    while not onGame:
        ...
        if EF_1_passed >= maxEnemiesPassed:
            ...
            drawText("Press  'N' to Quit", padWidth // 3, padHeight // 1.8, (255, 255, 255))
        ...
        if isShot and bossAppeared:
            ...
            if bossHP <= 0:
                ...
                pygame.display.update()
                pygame.time.wait(2000)  # 2초 대기 후 종료
                return  # 게임 종료
